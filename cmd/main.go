...
func main() {
	...
	metricsAddr := flag.String("metrics-bind-address", "0", "The address the metric endpoint binds to. Use the port :8080. If not set, it will be 0 in order to disable the metrics server")
	...
	leaderElectionId := flag.String("leader-election-id", "mobius-manager", "Determines the name of the resource that leader election will use for holding the leader lock.")

	loggerOptions := zap.Options{Development: !*debug}
	...
	if err = fieldindex.RegisterFieldIndexes(context.Background(), mgr.GetFieldIndexer()); err != nil {
		setupLog.Error(err, "failed to register field index")
		os.Exit(1)
	}

	if err = networkingcontroller.NewExternalProxyReconciler(mgr.GetClient(), mgr.GetScheme()).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ExternalProxy")
		os.Exit(1)
	}
